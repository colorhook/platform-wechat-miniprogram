...
import { systemInfo } from "@oasis-engine/wechat-adapter";
/**
...
export class OrbitControl extends Script {
  ...
  pan(deltaX: number, deltaY: number) {
    ...
    this.panLeft(-2 * deltaX * (targetDistance / systemInfo.windowWidth), this.camera.transform.worldMatrix);
    this.panUp(2 * deltaY * (targetDistance / systemInfo.windowHeight), this.camera.transform.worldMatrix);
  }
  ...
  handleTouchMoveRotate(event: TouchEvent) {
    ...
    this.rotateLeft(((2 * Math.PI * this._rotateDelta.x) / systemInfo.windowWidth) * this.rotateSpeed);
    this.rotateUp(((2 * Math.PI * this._rotateDelta.y) / systemInfo.windowHeight) * this.rotateSpeed);
